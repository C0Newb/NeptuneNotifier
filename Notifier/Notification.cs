...
using static Notifier.NotificationIcon;
...
namespace Notifier {
    public class Notification {
        ...
        public static void NewToast(XmlDocument toastXml, int id = -1, string activeArgs = "action=default") {
            if (id < 0) {
                LastNotificationId++;
                id = LastNotificationId;
            }

            ((XmlElement)toastXml.GetElementsByTagName("toast")[0]).SetAttribute("launch", id + ":" + activeArgs);
        ...
        public static void DeleteNotification(string tag) {
            ToastNotificationManager.History.Remove(tag);
        }
        public static void DeleteNotification(int tag) {
            ToastNotificationManager.History.Remove(tag.ToString());
        }
        ...
        private static void Toast_Failed(ToastNotification sender, ToastFailedEventArgs args) {
            ...
            NotificationActivator.Activate(APP_ID, new NotificationEventArgs(sender, reason, moreDetails));
        }
        private static void Toast_Dismissed(ToastNotification sender, ToastDismissedEventArgs args) {
            NotificationActivator.Activate(APP_ID, new NotificationEventArgs(sender, args));
        }
        private static void Toast_Activated(ToastNotification sender, object args) {
            if (args.GetType() == typeof(ToastActivatedEventArgs))
                NotificationActivator.Activate(APP_ID, new NotificationEventArgs(sender, (ToastActivatedEventArgs)args));
            else
                NotificationActivator.Activate(APP_ID, new NotificationEventArgs(sender, args.ToString()));
        }
        ...
        private static void Icon_BalloonTipClosed(object sender, EventArgs e) {
            ...
            NotificationActivator.Activate(APP_ID, notificationEventArgs);
        }
    ...
    public class NotificationEventArgs {
        ...
        public NotificationEventArgs(ToastNotification sender, ToastDismissedEventArgs args) {
            if (sender.Content.ChildNodes.Count <= 1) {
                int.TryParse(sender.Tag, out NotificationId);
